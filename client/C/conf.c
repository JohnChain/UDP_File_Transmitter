...
int init_conf(char *conf_filename)
{
    if(access(conf_filename, 0) == -1)
    {
        ...
        config_t *myconf = confRead(conf_filename);
        my_confSet(myconf, "filesize", 0);
        my_confSet(myconf, "costtime", 0);
        my_confSet(myconf, "filename", 0);
        confSet(myconf, "detail", "====");
...
//int block_num = 14;
//int taskArray[] = {3,3,3,3,3,3,1,1,1,1,0,0,0,0};
