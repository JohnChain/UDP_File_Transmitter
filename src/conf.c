...
config_t *newConfiglist(const char *file_name, size_t len)
{
...
config_t *confRead(const char *file_name)
{
	...
	config_t *one = newConfiglist(file_name, palen);
	if (!one) {
		fclose(fp);
		return NULL;
	}
...
int init_conf()
{
    ...
    sprintf(conf_filename, ".part_target.conf");
    if(access(conf_filename, 0) == -1)
    {
        FILE * temp_handler = fopen(conf_filename, "w+");
        fclose(temp_handler);

        config_t *myconf = my_confRead();
        my_confSet(myconf, "thread_num", 0);
        my_confSet(myconf, "blocksize", 0);
        my_confSet(myconf, "block_num", 0);
...
config_t *my_confRead()
{
    char filename_buf[LEN_FILENAME];
    sprintf(filename_buf, ".part_target.conf");
    return (config_t *)confRead(filename_buf);
}

int my_confGet(config_t *conf_hd, char *key)
{
    return (int)atoi(confGet(conf_hd, key));
}

int my_confSet(config_t *conf_hd, char *key, int val)
{
    char val_buf[LEN_FILENAME];
    sprintf(val_buf, "%d", val);
    confSet(conf_hd, key, val_buf);
    return 0;
}
...
//{
    //if(argc != 2)
    //{
        //printf("USAGE: conf filepath\n");
        //return 0;
    //}
    //FILE *conf_handler;
    //if(access(argv[1], NULL) == -1)
        //conf_handler = fopen(argv[1], "a+");
    //fclose(conf_handler);
    //init_conf(3);
	//return 0;
//}
